...
def add_labels(xs, int_values, offset, neg_offset=None):
    if neg_offset is None:
        neg_offset = offset
    for x, c in zip(xs, int_values):
        plt.text(x, c + offset if c >= 0 else c - neg_offset, c, ha="center")
...
def plot_weekday_and_month_side_by_side(dates_1, dates_2):
    ...
    for i, month_count in enumerate(month_counts):
        offset = -width / 2 - padding / 2 + (width + padding) * i
        add_labels([x + offset for x in range(12)], month_count, max_count * 0.03)
